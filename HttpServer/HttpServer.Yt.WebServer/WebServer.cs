...
namespace HttpServer.Yt.WebServer
{
    public class WebServer : Main.HttpServer
    {
        ...
        private Queue<(object, Type)> controllersToAdd;

        public WebServer(string hostUrl, string hostDir, ILogger logger)
            : base(hostUrl, hostDir, logger)
        {
            RouteTree = new RouteTree();
            controllersToAdd = new Queue<(object, Type)>();
        }

        public void RegisterController<T>(T obj) where T : Controller
        {
            if (running)
            {
            ...
            controllersToAdd.Enqueue((obj, typeof(T)));
        }
        ...
        protected override async Task Startup()
        {
            ...
            {
                var controllerAndType = controllersToAdd.Dequeue();
                Type controllerType = controllerAndType.Item2;
                object controllerObj = controllerAndType.Item1;
                ...
                foreach (var method in controllerType.GetMethods())
                {
                    ...
                    if (attr != null)
                    {
                        Console.WriteLine($"Registered method {method.Name} under route {attr.Method} {attr.Route}");
